...
3. 예시 경로로 `C:\Temp\aaa` 같은 테스트 폴더를 입력합니다.
4. 해당 폴더에 텍스트 파일을 하나 만듭니다.
5. `List files`로 파일 목록을 확인합니다.
6. `Save snapshot`으로 현재 상태를 저장합니다.
7. 파일을 수정하거나 새 파일을 추가하거나 기존 파일을 삭제합니다.
8. `Check status`로 변경사항을 확인합니다.
9. `Start realtime watch`를 실행한 뒤 탐색기에서 파일을 변경합니다.
10. `Show change log`로 기록된 변경 로그를 확인합니다.
